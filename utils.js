...
const utils = {
	handle_error(e, default_message) {
		// We don't want to just throw an error object at a user.. so when we reject promises we can give useful error messages. But in the event that it's not an error we're aware of, we still want a default message for that specific h5banner command.
	...
	read_path: async function (filePath) {
		// return file if exists, return false if it doesn't
		try {
	...
	vendorify: async function (config, banner_info, vendor_name, vendor_path) {
		const vendor = config.vendors[vendor_name],
			...
			destPath = "./" + config.project + "-preview/" + config.project + "-handoff/" + vendor_name + "/" + size,
			source_path = "./banners/" + size + ".html";

		try {
			let source = await fs.readFileAsync(source_path, "utf8");
			source = this.replace_string_regex(source, "<!-- (?:H5BANNER|ANIONE): vendorScriptHeader -->", vendor.scriptHeader);
			source = this.replace_string_regex(source, "<!-- (?:H5BANNER|ANIONE): vendorScriptFooter -->", vendor.scriptFooter);
			source = this.replace_string_regex(source, "#(?:H5BANNER|ANIONE):vendorLink", vendor.link);
			source = this.replace_string_regex(source, "../assets/images/" + size + "/", "");
			source = source.replace(/return \(function\(\) \{/, "(function() {");

			return Promise.resolve({ file: source, path: destPath, size: size });
		} catch (e) {
		...
		try {
			...
			for (let i = 0; i < files.length; i++) {
				...
				if (!this.is_hidden(filename)) {
					...
					let layer_name = camel(filename.split(".")[0].replace(/@\d+x$/i, ""));
					let width, height;
					try {
						const dimensions = sizeOf(`./assets/images/${size}/${filename}`);
						const scaleMatch = filename.match(/@(\d+)x\./i);
						const scale = scaleMatch ? parseInt(scaleMatch[1], 10) : 1;
						if (dimensions && dimensions.width && dimensions.height && scale > 0) {
							width = Math.round(dimensions.width / scale);
							height = Math.round(dimensions.height / scale);
						}
					} catch (e) {
						// skip width/height if the file isn't a readable image
					}
					img_array.push({ filename: filename, layer_name: layer_name, width: width, height: height });
				}
			}
			if (copy) {
				await imagemin([`./assets/images/${size}/*.{jpg,png,gif,svg}`], { destination: destination, plugins: [imageminJpegtran(), imageminPngquant({ quality: [0.6, 0.8] })] });
			}
	...
	process_templates: {
		banner: async function (config, image_list) {
			...
			const banner_file = await $.read_path(`./banners/${config.sizes[0]}.html`),
				...
				locals = { images: image_list, imgPath: $.paths.directories.images + config.sizes[0], width: config.sizes[0].split("x")[0], height: config.sizes[0].split("x")[1], pageTitle: config.sizes[0] },
		...
		dev: async function () {
			...
			try {
				...
				const options = { pretty: true, filename: "index.html" },
					locals = { banners: banner_files, scrubber: scrubber },
					html = pug.renderFile(templatePath, Object.assign(options, locals));
		...
		preview: async function (previewDir) {
			...
			previewDir = previewDir || `./${config.project}-preview`;
			...
			try {
				...
				animated_banners = await Promise.all(
					animated_banners.map(async (banner) => {
						const newPath = banner.path.replace(/\.\/banners\//gi, "banners/"),
						...
						return (banner = Object.assign(banner, { path: newPath, width: size.split("x")[0], height: size.split("x")[1], fileSize: fileSize, animated: true }));
					}),
				);

				static_banners = await Promise.all(
					static_banners.map(async (banner) => {
						const newPath = banner.path.replace(/\.\/assets\//gi, "assets/");
						let fileSize = fs.statSync(newPath)["size"];
						fileSize = (fileSize / 1024).toFixed(2) + "kB";

						return (banner = Object.assign(banner, { path: newPath, fileSize: fileSize, animated: false }));
					}),
				);
	...
	watch: async function () {
		...
		browserSync.init({ server: { baseDir: "./" }, files: this.paths.watch, logPrefix: config.project, reloadOnRestart: true, notify: false });
	...
	get_files_in: async function (filePath) {
		try {
			...
			for (let filename of files) {
				if (filename && !this.is_hidden(filename)) {
					// we don't want hidden files
					let layer_name = camel(filename.split(".")[0]);
					files_array.push({ filename: filename, layer_name: layer_name, path: filePath + filename });
				}
			}
	...
	paths: {
		build_directories: [
			// these are the directories that are generated during `h5banner init`
			"./assets",
			"./assets/statics",
			"./assets/images",
			"./banners",
		],
		directories: { images: "../assets/images/" },
		template: { banner: "templates/banner.pug", dev: "templates/dev.pug", preview: "templates/preview.pug" },
		watch: ["banners/**", "assets/**", "index.html"],
	},
};
