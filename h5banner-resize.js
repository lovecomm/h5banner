...
exports.resize = async function () {
	...
	try {
		...
		if (!first_banner.file) return $.handle_error("Your first banner couldn't be found. Run 'h5banner one' to generate your first banner.");

		for (let size of config.sizes) {
			if (size != first_banner.size) {
				let banner_file = await $.read_path(`./banners/${size}.html`);
				if (banner_file) {
					$.handle_notice(`The banner, ${size}.html already exists so it will not be regenerated. To regenerate it, delete the file and re-run 'h5banner resize'`);
				} else {
					let banner = {
						size: size,
