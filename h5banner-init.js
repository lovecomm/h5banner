...
const Promise = require("bluebird"),
	...
	configQuestions = (vendors, sizes) => {
		return [
			...
			{
				type: "checkbox",
				message: "Select Banner Sizes. If you do not see all of the required sizes for your project here, you can add them after the project initialization by editing h5banner-conf.js directly.\n",
				name: "sizes",
				choices: sizes,
				validate: (answer) => {
			...
			{
				type: "checkbox",
				message: "Select Vendors. If you do not see all of the required vendors for your project here, you can add them after the project initialization by editing h5banner-conf.js directly.\n",
				name: "vendors",
...
exports.init = async function () {
	const config_file = await $.read_path("./h5banner-conf.json"),
		readme_file = await $.read_path("./README.md"),
		gitignore_file = await $.read_path("./.gitignore");

	if (config_file) return $.handle_notice("Your project has already been initialized. To re-initialize, first delete h5banner-conf.json.");
	...
	try {
		...
		await fs.writeFileAsync("h5banner-conf.json", config);
		...
		$.handle_success(`Your project has been configured! You can edit 'h5banner-conf.json' manually to correct errors.`);
